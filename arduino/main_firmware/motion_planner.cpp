...
bool MotionPlanner::startPick(int x, int y, int z, float confidence,
                              String class_type, bool isSimulation,
                              int object_height_mm) {
...
void MotionPlanner::update() {
  ...
  if (!_servoMgr->isMoving() && _currentState != PICK_GRASP &&
  ...
  case PICK_CALCULATE_POSE:
    ...
    // Adjust pick and place depths based on object height.
    {
      ...
      _frontPickPose.shoulder =
          constrain(22 + heightDelta, LIMIT_SHOULDER_MIN, LIMIT_SHOULDER_MAX);
      _frontApproachPose.shoulder =
          constrain(35 + heightDelta, LIMIT_SHOULDER_MIN, LIMIT_SHOULDER_MAX);
      _frontLiftPose.shoulder =
  ...
  case PICK_WAIT_CLOSE_CLAW:
    if (millis() - _stateStartTime >
        1000) { // Slightly longer wait for firm grip
  ...
  case PICK_MOVE_TRANSIT:
    ...
    _servoMgr->setTargets(_transitPose.waist, _transitPose.shoulder,
                          _transitPose.elbow, _transitPose.wrist_roll,
                          _frontPickPose.wrist_pitch, _targetClawAngle);
  ...
  case PICK_MOVE_BACK_APPROACH:
    if (_servoMgr->isMoving())
      return;
    _servoMgr->setTargets(_backApproachPose.waist, _backApproachPose.shoulder,
                          _backApproachPose.elbow, _backApproachPose.wrist_roll,
                          _frontPickPose.wrist_pitch, _targetClawAngle);
  ...
  case PICK_MOVE_PLACE_DOWN:
    if (_servoMgr->isMoving())
      return;
    _servoMgr->setTargets(_backPlacePose.waist, _backPlacePose.shoulder,
                          _backPlacePose.elbow, _backPlacePose.wrist_roll,
                          _frontPickPose.wrist_pitch, _targetClawAngle);
  ...
  case PICK_ADJUST_PITCH:
    if (_servoMgr->isMoving())
      return;
    if (millis() - _stateStartTime < 600)
      return; // Wait for arm to settle
...
PickState MotionPlanner::getState() { return _currentState; }
...
void MotionPlanner::reset() { _currentState = PICK_IDLE; }

String MotionPlanner::getLastError() { return _lastError; }

void MotionPlanner::setApproachOffset(int mm) { _approachOffsetMm = mm; }

void MotionPlanner::setGraspDistance(int mm) { _graspDistanceMm = mm; }

void MotionPlanner::setLiftHeight(int degrees) { _liftHeightDeg = degrees; }
...
void MotionPlanner::calculateTargetPose() {
  // Calculate waist angle from cartesian coordinates (maps pixel X to base
  // rotation)
  ...
  if (reach > 290 && reach < 310 && _targetX == 0) {
    ...
    _targetShoulderAngle =
        constrain(96 - shoulder_deg, LIMIT_SHOULDER_MIN, LIMIT_SHOULDER_MAX);
    _targetElbowAngle =
        constrain(180 - elbow_deg, LIMIT_ELBOW_MIN, LIMIT_ELBOW_MAX);
  }
...
bool MotionPlanner::moveToPose(const BinPose &pose, int clawAngle) {
  ...
  _servoMgr->setTargets(pose.waist, pose.shoulder, pose.elbow, pose.wrist_roll,
                        pose.wrist_pitch, clawAngle);
  return true;
}
...
int MotionPlanner::calculateApproachPose(int target_waist,
                                         int target_shoulder) {
  // Calculate a safe approach pose offset from target
  // For now, just return a slightly different shoulder angle
  return constrain(target_shoulder - 20, LIMIT_SHOULDER_MIN,
                   LIMIT_SHOULDER_MAX);
}
...
String MotionPlanner::_stateToName(PickState state) {
  ...
  case PICK_MOVE_TRANSIT:
    return "MOVE_TRANSIT";
  case PICK_MOVE_BACK_APPROACH:
    return "MOVE_BACK_APP";
  case PICK_MOVE_PLACE_DOWN:
    return "MOVE_PLACE_DOWN";
  case PICK_ADJUST_PITCH:
    return "ADJUST_PITCH";
  case PICK_RELEASE:
    return "RELEASE";
